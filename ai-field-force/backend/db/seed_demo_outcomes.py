...
demo rep's territory so dashboards look populated. Safe to call on every
startup — uses sentinel client_outcome_id values so re-running won't duplicate.

Outcomes spread across the last 2 weeks. Mix of sale / follow-up / complaint
to make the territory_health_score look like a real product in use.
"""
import uuid
...
# Each tuple: (entity_id, days_ago, rating, type, actions, notes)
# Spread across the last 14 days; mix of outcome types.
_DEMO_OUTCOMES = [
...
def seed_demo_outcomes(db: Session) -> dict:
    ...
    # Ensure the demo device exists (it backs all seeded outcomes for idempotency)
    rep_row = None
    ...
    for i, (entity_id, days_ago, rating, otype, actions, notes) in enumerate(_DEMO_OUTCOMES):
        # Stable client_outcome_id so re-running is idempotent
        ...
        # Validate entity exists in territory
        entity = db.query(FarmerRetailer).filter(FarmerRetailer.id == entity_id).first()
        if not entity:
            skipped += 1
            continue

        when = datetime.utcnow() - timedelta(days=days_ago, hours=i)  # spread within the day too
        db.add(Outcome(
    ...
    if created > 0:
        try:
            ...
            current = 0
        device.sync_count = str(current + created)
